package com.palette.palettepetsback.articleWrite.controller;
...
import com.palette.palettepetsback.articleWrite.service.ArticleWriteService;
...
public class ArticleWriteController {
    ...
    public ResponseEntity<ArticleWrite> create(@Valid @RequestBody ArticleWriteDto dto){
        ArticleWrite created = articleWriteService.create(dto);
        return (created != null)?
    ...
    public ResponseEntity<ArticleWrite> update( @PathVariable Long id,
                                                @Valid
                                              @RequestBody ArticleWriteDto dto
    ){
        ArticleWrite updated = articleWriteService.update(id,dto); // 서비스를 통해 게시글 수정
    ...
    public ArticleWrite delete(@PathVariable Long id){
        // 1. 대상 찾기
